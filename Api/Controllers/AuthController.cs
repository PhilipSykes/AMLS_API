...
public class AuthController : ControllerBase
{
    ...
    public async Task<ActionResult<Response<LoginDetails>>> Login([FromBody] Request<PayLoads.Login> request)
    {
        ...
        {
            new(DbFieldNames.Login.Email, request.Data.Email, DbOperations.Equals)
        };
        var result = await _userSearch.GetLoginCredentials(emailFilter);
        ...
        var auth = new TokenAuthService();
        var token = auth.GenerateJwtToken(result[0]);
