namespace Common.Constants;

public static class PolicyClaims
{
    public const string AdminClaim = "admin_access";
    public const string LibrarianClaim = "librarian_access";
    public const string ManagerClaim = "manager_access";
    public const string AccountantClaim = "accountant_access";
    public const string MemberClaim = "member_access";
    public const string BranchAccess = "branch_access";
    
    public const string ReadMedia = "read_media";
    public const string CreateMedia = "create_media";
    public const string EditMedia = "edit_media";
    public const string DeleteMedia = "delete_media";
    public const string ManageUsers = "manage_users";
    public const string ViewFinancialReports = "view_financial_reports";
    public const string ViewStaffReports = "view_staff_reports";
    public const string ViewMetricsReports = "view_metrics_reports";
    
}